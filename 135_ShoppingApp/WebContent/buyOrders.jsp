		...
		<% if(request.getParameter("totalOrder") != null && !request.getParameter("totalOrder").equals("")) {
			try {
				if(Integer.parseInt(request.getParameter("totalOrder")) <= 0 ) {
					...
					String log_sql = "INSERT INTO logtable(person_id, product_id, amount) VALUES(?, ?, ?)"; 
					String log_final = "insert into logtable2 ( select  s.state_name, p.product_name, sum(lg.amount)  from (logtable left join person as p on p.id = person_id ) "+ 
									   "as lg left join state as s on lg.state_id = s.id "+
										"left join product as p on p.id = lg.product_id " +
					...
					int productPrice = 0;
					int quantity = 0;			
					PreparedStatement shoppingCartPtst = null, productsCartPtst  = null, logtablePtst = null;
					Statement personSt = null, productSt = null, log_st = null;
					...
					try {
						...
						logtablePtst = con.prepareStatement(log_sql);
						...
						for(int i=0;i<noOfSales;i++) {
							...
							while(productRs.next()) {
								...
								logtablePtst.setInt(1, pid.get(i));
								logtablePtst.setInt(2,productId);
								logtablePtst.setInt(3, productPrice*quantity);
								productsCartPtst.addBatch();
								logtablePtst.addBatch();
								totalRows++;
								
								if(totalRows % batchSize == 0) {
									productsCartPtst.executeBatch();
									logtablePtst.executeBatch();
								}
							}
							productsCartPtst.executeBatch();
							logtablePtst.executeBatch();
						 ...
						 log_st.executeUpdate(log_final);
